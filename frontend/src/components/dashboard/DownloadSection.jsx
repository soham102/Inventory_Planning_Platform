...
export const DownloadSection = ({ jobId, rowCount, replenishQty }) => {
  ...
  return (
    <div className="rounded-lg bg-slate-900 border border-slate-800 p-6 flex flex-col lg:flex-row items-start lg:items-center gap-6 justify-between">
      <div>
        <div className="flex items-center gap-2 text-emerald-300 mb-2">
          <CheckCircle2 className="w-4 h-4" />
          <span className="text-[10px] uppercase tracking-[0.18em] font-bold">Operations ready</span>
        </div>
        <h3 className="font-display text-xl font-bold tracking-tight text-slate-100">
          Final Replenishment Order Sheet
        </h3>
        <p className="text-sm text-slate-400 mt-1">
          <span className="font-mono-num text-slate-200">{rowCount?.toLocaleString()}</span> SKU recommendations ·{" "}
          <span className="font-mono-num text-cyan-300">{replenishQty?.toLocaleString()}</span> units to dispatch
        </p>
      </div>
      <div className="flex flex-col sm:flex-row gap-3 w-full lg:w-auto">
        ...
        <Button
          ...
          className="bg-slate-800 hover:bg-slate-700 border border-slate-700 text-slate-50 font-semibold"
        >
