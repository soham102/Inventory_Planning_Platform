...
export const Sidebar = () => {
  return (
    <aside
      ...
      className="hidden md:flex flex-col fixed left-0 top-0 h-screen w-64 bg-slate-950 border-r border-slate-800 z-30"
    >
      <div className="h-16 flex items-center gap-3 px-5 border-b border-slate-800">
        <div className="relative">
          <div className="w-8 h-8 rounded-md bg-cyan-500/15 border border-cyan-400/40 flex items-center justify-center">
            <Radar className="w-4 h-4 text-cyan-300" strokeWidth={2.25} />
          </div>
          <span className="absolute -right-0.5 -top-0.5 w-2 h-2 rounded-full bg-emerald-400 animate-pulse-dot" />
        </div>
        <div className="leading-tight">
          <div className="font-display font-black text-sm tracking-tight text-slate-50">COMMAND CENTER</div>
          <div className="text-[10px] uppercase tracking-[0.18em] text-slate-500">Inventory OS</div>
        </div>
      </div>

      <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
        <div className="px-2 pb-2 text-[10px] uppercase tracking-[0.18em] text-slate-500">Operations</div>
      ...
      <div className="m-3 p-3 rounded-lg border border-slate-800 bg-slate-900/60">
        <div className="flex items-center gap-2">
          <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse-dot" />
          <span className="text-[10px] uppercase tracking-[0.18em] text-emerald-300 font-bold">System Online</span>
        </div>
        <div className="mt-2 text-xs text-slate-400">All pipelines operational</div>
        <div className="mt-2 font-mono-num text-[11px] text-slate-500">v1.0 · sync 12:42 UTC</div>
      </div>
    </aside>
  );
};
