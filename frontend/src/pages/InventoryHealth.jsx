  ...
  return (
    <div className="space-y-6 animate-fade-up">
      ...
      <div className="rounded-lg bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-4">
        <h3 className="font-display font-bold text-base text-slate-900 dark:text-slate-100 mb-3">City Breakdown</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
          {cities.map((c) => (
            <button
              ...
              className="text-left rounded-lg border border-slate-200 dark:border-slate-800 bg-slate-50/60 dark:bg-slate-950/40 p-3 hover:border-cyan-500/50 hover:bg-slate-50 dark:hover:bg-slate-900 transition-all group"
            >
              <div className="flex items-center justify-between">
                <div className="font-display font-bold text-slate-900 dark:text-slate-100">{c.city}</div>
                <div className="flex items-center gap-2">
                  <span className="font-mono-num text-xs text-slate-500 dark:text-slate-500 dark:text-slate-500 dark:text-slate-400">{c.sku_count} SKUs</span>
                  <ChevronRight className="w-3.5 h-3.5 text-slate-500 dark:text-slate-500 dark:text-slate-400 dark:text-slate-600 group-hover:text-cyan-300 group-hover:translate-x-0.5 transition-all" />
                </div>
              </div>
              <div className="mt-3 h-1.5 w-full bg-slate-200 dark:bg-slate-800 rounded-full overflow-hidden flex">
              ...
              <div className="mt-2 flex items-center justify-between text-xs font-mono-num">
                <span className="text-red-600 dark:text-red-300">H {c.high_risk_count}</span>
                <span className="text-amber-600 dark:text-amber-300">M {c.medium_risk_count}</span>
                <span className="text-emerald-600 dark:text-emerald-300">L {c.low_risk_count}</span>
              </div>
            </button>
          ))}
        </div>
