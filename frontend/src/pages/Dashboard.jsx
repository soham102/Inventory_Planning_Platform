  ...
  return (
    <div data-testid={DASH.loaded} className="space-y-6 animate-fade-up">
      <PageHeader
        ...
        tag="LIVE FEED"
        right={
          <div className="hidden md:flex items-center gap-2 text-xs text-slate-500 dark:text-slate-500 dark:text-slate-500 dark:text-slate-400">
            <FileSpreadsheet className="w-3.5 h-3.5 text-cyan-600 dark:text-cyan-300" />
...
const PageHeader = ({ title, subtitle, tag, right }) => (
  <div className="flex items-start justify-between gap-4 flex-wrap">
    <div>
      <div className="text-[10px] uppercase tracking-[0.2em] font-bold text-cyan-600 dark:text-cyan-300">{tag}</div>
      <h1 className="font-display text-2xl sm:text-3xl font-black tracking-tight text-slate-900 dark:text-slate-50 mt-1">
        {title}
      </h1>
      <p className="text-sm text-slate-500 dark:text-slate-500 dark:text-slate-500 dark:text-slate-400 mt-1 max-w-2xl">{subtitle}</p>
    </div>
    {right}
  </div>
);
...
const ExplainBanner = () => (
  <div className="rounded-lg border-l-2 border-cyan-400 bg-slate-200/40 dark:bg-slate-800/40 p-4 flex items-start gap-3">
    ...
    <div className="text-sm text-slate-700 dark:text-slate-300">
      <span className="font-display font-bold text-slate-900 dark:text-slate-100">AI reasoning is on.</span>{" "}
